...
const CandidateDashboard: React.FC<CandidateDashboardProps> = ({ onBrowse, user, allJobs }) => {
  const tabs = ['overview', 'applications', 'saved'] as const;
  ...
  const savedJobs = useMemo(() => {
    ...
    return allJobs.filter(job => user.savedJobIds?.includes(job.id) ?? false);
  }, [user, allJobs]);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
        ...
        <div className="flex items-center gap-3">
          <span className="px-4 py-2 rounded-full border border-emerald-200 bg-emerald-50 text-[10px] font-black uppercase tracking-widest text-emerald-700">
      ...
      <div className="mb-10 flex flex-wrap gap-3">
        {[
          { label: "Update profile", action: () => setShowAuditModal(true) },
          { label: "View saved jobs", action: () => setActiveTab("saved") },
          { label: "Browse roles", action: onBrowse },
        ].map((item) => (
          <button
            key={item.label}
            onClick={item.action}
            className="px-4 py-2 rounded-full border border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-500 hover:border-indigo-200 hover:text-indigo-600"
          >
            {item.label}
          </button>
        ))}
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-indigo-600 font-bold text-sm uppercase tracking-widest mb-6">Operations Talent</p>
             ...
             <div className="space-y-4 animate-in fade-in">
                {savedJobs.length > 0 ? savedJobs.map(job => (
                   <div
                     ...
                     tabIndex={0}
                     onClick={() => {
                       window.location.href = `/jobs/${createJobSlug(job)}`;
                     }}
                     onKeyDown={(event) => {
                     ...
                     className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-lg transition-all flex items-center justify-between group cursor-pointer"
                   >
                      ...
                      <button
                        ...
                        disabled={!job.applyUrl || job.applyUrl === '#'}
                        className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-black text-xs hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-100 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                         {job.applyUrl && job.applyUrl !== '#' ? 'Apply' : 'Apply soon'}
                      </button>
                   </div>
                )) : (
