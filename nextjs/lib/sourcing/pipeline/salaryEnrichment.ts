...
import { normalizeHost, normalizeUrlForRegistry } from "../domain";
import { detectAndParseSalaryForGreenhouse, parseSalaryFromText } from "../normalization/salary";
...
export type SalaryEnrichmentArgs = {
...
export type SalaryEnrichmentSummary = {
  ok: true;
  scanned: number;
  attempted: number;
...
export async function enrichCandidatesSalaryBulk(sb: AdminSb, args: SalaryEnrichmentArgs): Promise<SalaryEnrichmentSummary> {
  const sourceId = (args.sourceId || "").trim();
  const runId = (args.runId || "").trim();
  const limit = clampInt(args.limit, 1, 80, 30);
  const concurrency = clampInt(args.concurrency, 1, 5, 3);
  const cooldownHours = clampInt(args.cooldownHours, 0, 168, 24);

  // Safety: if caller didn’t narrow by run or source, keep scans small.
  const scanLimit = clampInt(args.scanLimit, 50, 800, sourceId || runId ? Math.max(200, limit * 8) : 120);
  ...
  if (error) throw new Error(`Unable to load candidates: ${error.message}`);
  ...
  const results = await asyncPool(concurrency, toAttempt, async (c) => {
    ...
    if (!parsed.ok) {
      await sb
        ...
        .eq("id", id);
      return { id, status: "failed" as const, reason: parsed.kind };
    }
    ...
    if (upErr) return { id, status: "failed" as const, reason: "update_failed" as const };
    return { id, status: "updated" as const, reopened: shouldReopenForPublish };
  });
